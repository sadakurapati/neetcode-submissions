class Solution {
    ...
    public boolean isInterleave(String s1, String s2, String s3) {
        ...
        return canFormFromHere(new WordProcessingState(s1, 0), new WordProcessingState(s2, 0), new WordProcessingState(s3, 0));
    }

    private boolean canFormFromHere(WordProcessingState inputWord1, WordProcessingState inputWord2, WordProcessingState targetWord) {
        ...
        if (inputWord1.isCurrentIndexValid() && inputWord1.getCurrentChar() == targetWord.getCurrentChar()) {
            res = canFormFromHere(new WordProcessingState(inputWord1.word(), inputWord1.currentIndex() + 1), inputWord2, new WordProcessingState(targetWord.word(), targetWord.currentIndex() + 1));
        }
        if (!res && inputWord2.isCurrentIndexValid() && inputWord2.getCurrentChar() == targetWord.getCurrentChar()) {
            res = canFormFromHere(inputWord1, new WordProcessingState(inputWord2.word(), inputWord2.currentIndex() + 1), new WordProcessingState(targetWord.word(), targetWord.currentIndex() + 1));
        }
